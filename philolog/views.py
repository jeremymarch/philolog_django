...
def query(request):
    ...
    if query_data.page <= 0:
        before_words = Words.objects.filter(word__lt='αβγ').order_by('word')[0:100]
    if query_data.page >= 0:
        after_words = Words.objects.filter(word__gte='αβγ').order_by('-word')[0:100]

    words = list(before_list) + list(after_words)  # https://stackoverflow.com/questions/4424435/how-to-convert-a-django-queryset-to-a-list
